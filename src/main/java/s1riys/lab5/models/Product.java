...
public class Product extends ModelWithId implements Comparable<Product>, Validatable {
    ...
    private String partNumber; // Length of the string should be at least 21, Value of this field should be unique, Field can be null
        ...
        try {
            ...
            if (manufacturerIsNull) {
                ...
                manufacturer = new Organization(
                        listOfValues[8],
                        listOfValues[9],
                        Double.parseDouble(listOfValues[10]),
                        Integer.parseInt(listOfValues[11])
                );
            }

            Product product = new Product(name, new Coordinates(coordinateX, coordinateY), creationDate, price, partNumber, unitOfMeasure, manufacturer);
            product.setId(id);
            return product;
